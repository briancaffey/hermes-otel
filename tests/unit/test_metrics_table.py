...
class TestCreation:
    ...
    def test_a_failing_instrument_does_not_take_the_others_down(self):
        ...
        p._instruments["token_usage"].add.assert_called_once_with(7, {"token_type": "input"})


class TestRecording:
    def test_value_modes(self, plugin):
        plugin.record_metric("session_count", 99, {"platform": "cli"})
        plugin._instruments["session_count"].add.assert_called_once_with(1, {"platform": "cli"})
        plugin.record_metric("token_usage", 12.9, {"token_type": "input"})
        plugin._instruments["token_usage"].add.assert_called_once_with(12, {"token_type": "input"})
        plugin.record_metric("cost_usage", 0.0042, {"model": "m"})
        plugin._instruments["cost_usage"].add.assert_called_once_with(0.0042, {"model": "m"})
    ...
    def test_live_store_gets_the_otlp_name(self, plugin, monkeypatch):
        ...
        assert name == "hermes.token.usage" and value == 3 and attrs == {"token_type": "output"}
