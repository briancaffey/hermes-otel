...
- `hermes.tool.outcome` — one of `completed` · `error` · `timeout` · `blocked` · `cancelled` · (explicit `status` field from the result, lowercased). Hermes' own `post_tool_call` lifecycle `status` is authoritative for non-success outcomes (`timeout`, `blocked`, `cancelled`, `error` — the result may be plain text there); for `ok` the result's own status is kept, else `completed`. Only `error` maps the span `StatusCode` to `ERROR`; timeouts/blocked/cancelled stay `OK` so dashboards don't count them as failures.
